...
def BA_Homography(src_pts, dst_pts, weights, init_guess):
...
def main():
    ...
    weights = torch.ones(src_pts.shape).float()
    weights[2:, :] = 1
    ...
    weights = weights[None, :, :].repeat(batch_size, 1, 1).cuda()

    st = time.time()
    iter_num = 1
    for i in range(iter_num):
        h_est = BA_Homography(src_pts, dst_pts, weights, theta0)
    et = time.time()
